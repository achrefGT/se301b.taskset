...
int create_fp_thread(unsigned int priority, unsigned int stack_size,
		void* (*start_routine)(void), pthread_t *tid, int policy) {
	int sys_ret;
	pthread_attr_t attr;
	...
	sys_ret = pthread_create(tid, &attr, (void* (*)(void*)) start_routine,
			NULL);
